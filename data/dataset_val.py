from torch.utils.data import Dataset
from .preprocess import *
import os
import glob
import numpy as np
import random

class LipreadingDataset_val(Dataset):

    def __init__(self, data_root, index_root, padding, augment=True, pinyins=None, **kwargs):
        self.padding = padding
        self.data = []
        self.data_root = data_root
        self.padding = padding
        self.augment = augment
        self.tot_time = 0

        index_root = "D:/Datasets/LRW-1000/MAVSR2019_val_task3_kws/kws/images"

        self.data_root = index_root 

        self.video_list = sorted(os.listdir(index_root))

        output_file = "C:/Users/YorkeYao/Desktop/MAVSRC/checkpoint_prob"

        # with open(output_file, 'r', encoding='utf-8') as f:
        #     lines = f.readlines()
        #     used_video_list = [line.strip().split(' ')[0] for line in lines]

        print (len (self.video_list))

        used_video_list = [i.split('.')[0] for i in os.listdir(output_file)]
        for i in used_video_list:
            if i in self.video_list:
                self.video_list.remove (i)
        # print ( len (self.video_list - used_video_list))

        # self.video_list = self.video_list[-656:]
        print (len (self.video_list))
        # assert (0)

        for video in self.video_list:
            video_images = os.listdir (os.path.join(index_root, video))

            # print (len(video_images))

            for i in range (1, len(video_images) + 1):
                for j in range (3, 15):
                    if i + j < len(video_images) + 1:
                        self.data.append( ( video, i, i + j, None, len(video_images) ) )
        self.lengths = [data[2]-data[1] for data in self.data]
        # print (len (self.data))
        # assert (0)
        
        # with open(index_root, 'r', encoding='utf-8') as f:
        #     lines = f.readlines()
        #     lines = [line.strip().split(',') for line in lines]
        #     pinyins = sorted(np.unique([line[2] for line in lines]))
        #     self.tot_time = sum ([float(line[4]) - float(line[3]) for line in lines])
        #     self.data = [(line[0], int(float(line[3])*25)+1, int(float(line[4])*25)+1, pinyins.index(line[2])) for line in lines]
        #     self.data = list(filter(lambda data: data[2]-data[1] <= self.padding, self.data))
        #     self.lengths = [data[2]-data[1] for data in self.data]
        #     self.pinyins = pinyins
            
        # print('index file:',index_root)
        # print('num of pinyins:',len(pinyins))
        # print('num of data:',len(self.data))
        # print('max video length',np.max(self.lengths))
        # print('tot time', self.tot_time)

        #return pinyins
    
    
    def __len__(self):
        return len(self.data)

    def get_pinyins(self):
        return self.pinyins

    def get_video_list(self):
        return self.video_list

    def __getitem__(self, idx):
        #load video into a tensor
        (path, op, ed, label, video_length) = self.data[idx]         
        vidframes = load_images(os.path.join(self.data_root, path), op, ed)
        #print (np.shape (vidframes[0]))
        length = len(vidframes)
        temporalvolume = bbc(vidframes, self.padding, self.augment)
        return {'temporalvolume': temporalvolume, 
            'length': length,'op': op, 'ed':ed, 'path': path, 'video_length': video_length}


if __name__ == "__main__":
    a = LipreadingDataset_val("","", "")
